...
public interface ProtocolEntity
{
    ...
    static Options getOptions(Class<?> pclass, String xml)
    {
        if(pclass.isAnnotationPresent(ServerProtocol.class))
            return new Options(pclass.getAnnotation(ServerProtocol.class).name(), xml);
        if(pclass.isAnnotationPresent(ClientProtocol.class))
            return new Options(pclass.getAnnotation(ClientProtocol.class).name(), xml);
        return null;
    }
}
